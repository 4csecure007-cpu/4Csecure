...
const DocumentUpload = ({ onDocumentUploaded }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Simulate progress for better UX with slower increments to avoid getting stuck
      ...
      // Set a timeout to handle stuck uploads (especially for large/landscape PDFs)
      uploadTimeout = setTimeout(() => {
        console.warn('Upload taking longer than expected, continuing...')
      ...
      const { error: uploadError } = await supabase.storage
        .from('documents')
        .upload(filePath, formData.file, uploadOptions)
      ...
      const timeoutPromise = new Promise((_, reject) =>
        setTimeout(() => reject(new Error('Database insert timed out after 30 seconds. This may be due to Row Level Security policies blocking the insert.')), 30000)
      )

      const { data, error: dbError } = await Promise.race([dbInsertPromise, timeoutPromise])
      ...
      if (dbError) {
        ...
        if (dbError.code === 'PGRST116') {
          throw new Error('Permission denied: Unable to insert document. Please check your admin permissions and Row Level Security policies.')
        }
      ...
      if (error.message?.includes('row size') || error.message?.includes('payload')) {
        errorMessage += 'File is too large. Please try a smaller file or compress the PDF.'
      } else if (error.message?.includes('timeout') || error.message?.includes('timed out')) {
        errorMessage += 'Upload timed out. This usually means Row Level Security policies are blocking the insert. Please check:\n\n'
        errorMessage += '1. You are logged in as an admin\n'
        errorMessage += '2. RLS policies on the "documents" table allow INSERT for admins\n'
        errorMessage += '3. Your Supabase connection is working\n\n'
        errorMessage += 'Technical details: ' + error.message
      } else if (error.message?.includes('Permission denied') || error.message?.includes('RLS') || error.code === 'PGRST116') {
        errorMessage += 'Permission denied. Your account does not have permission to upload documents.\n\n'
        errorMessage += 'Please ensure:\n'
        errorMessage += '1. You are logged in as an admin user\n'
        errorMessage += '2. Row Level Security policies in Supabase allow document uploads for admin users\n'
        errorMessage += '3. The documents table has proper INSERT policies configured'
      } else if (error.message?.includes('violates foreign key') || error.message?.includes('user_id')) {
        errorMessage += 'Database error: user_id constraint failed.\n\n'
        errorMessage += 'Please ensure you are logged in and try again.'
      } else if (!user || !user.id) {
        errorMessage += 'You must be logged in to upload documents. Please refresh the page and try again.'
      } else {
